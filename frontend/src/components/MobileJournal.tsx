...
const generateAIFeedback = (entry: JournalEntry): string => {
  ...
  if (isWin && hasNeg) feedback += `�š�️ **Emotional Warning:** Profit while feeling ${entry.emotional_state} can reinforce bad habits.\n\n`;
  else if (!isWin && hasNeg) feedback += `�Ÿ”� **Emotional Risk:** ${entry.emotional_state} state likely contributed to this loss.\n\n`;
  else if (hasPos) feedback += `�œ… **Emotional Discipline:** ${entry.emotional_state} mindset �€” keep this up.\n\n`;
  if (pnl > 0) feedback += `�Ÿ’� **Result:** +$${pnl.toFixed(2)} �€” document what you did right.\n\n`;
  else if (pnl < 0) feedback += `�Ÿ“‰ **Loss:** -$${Math.abs(pnl).toFixed(2)} �€” review your entry criteria.\n\n`;
  if (lessons && lessons.length > 10) feedback += `�Ÿ“š **Learning:** Great job documenting lessons. Review these weekly.\n\n`;
  else if (!lessons) feedback += `�Ÿ’� **Tip:** Add lessons learned for every trade �€” even winners.\n\n`;
  let score = 50;
  ...
  feedback += `${score >= 70 ? '�ŸŸ�' : score >= 50 ? '�ŸŸ�' : '�Ÿ”�'} **Score: ${score}/100**`;
  return feedback;
};
...
const MobileFeedback: React.FC<{ entry: JournalEntry }> = ({ entry }) => {
  ...
  if (!shown) return (
    <Button onClick={get} disabled={loading} size="small" startIcon={loading ? <CircularProgress size={12} /> : <AutoAwesome sx={{ fontSize: 12 }} />}
      ...
      {loading ? 'Analyzing...' : '�œ� AI Feedback'}
    </Button>
  );
  return (
    <Box sx={{ mt: 1.5, p: 1.5, borderRadius: '10px', background: 'rgba(168,85,247,0.08)', border: '1px solid rgba(168,85,247,0.2)' }}>
      <Typography sx={{ fontSize: '10px', fontWeight: 700, color: '#a855f7', mb: 1, textTransform: 'uppercase', letterSpacing: '0.08em' }}>�œ� AI Analysis</Typography>
    ...
    <Box sx={{ minHeight: '100vh', background: 'linear-gradient(180deg, #0a0f1e 0%, #050810 100%)', pb: 10, pt: 2 }}>
      ...
      <Box sx={{ px: 2, mb: 2 }}>
        ...
        <Box sx={{ display: 'flex', gap: 0.8, mb: 1.5, flexWrap: 'wrap' }}>
          {[{ v: 'all', l: 'All' }, { v: 'wins', l: '�œ… Wins' }, { v: 'losses', l: '�Œ Losses' }].map(f => (
        ...
        <AnimatePresence mode="popLayout">
          {displayed.length === 0 ? (
            <Box sx={{ py: 8, textAlign: 'center' }}>
              <Typography sx={{ fontSize: '40px', opacity: 0.15, mb: 1 }}>�Ÿ““</Typography>
              ...
              <motion.div key={entry.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0, transition: { delay: i * 0.04 } }} exit={{ opacity: 0 }} layout>
                <Box sx={{ p: 2.5, mb: 2, borderRadius: '18px', background: hasPnl ? (isWin ? 'rgba(34,197,94,0.07)' : 'rgba(239,68,68,0.07)') : 'rgba(255,255,255,0.03)', border: `1px solid ${hasPnl ? (isWin ? 'rgba(34,197,94,0.2)' : 'rgba(239,68,68,0.2)') : 'rgba(255,255,255,0.07)'}`, position: 'relative', overflow: 'hidden' }}>
                  ...
                  {entry.lessons_learned && <Box sx={{ p: 1.2, borderRadius: '8px', background: 'rgba(251,191,36,0.08)', border: '1px solid rgba(251,191,36,0.2)', mb: 1.5 }}>
                    <Typography sx={{ fontSize: '10px', color: '#fbbf24', fontWeight: 600, mb: 0.3 }}>�Ÿ’� LESSONS</Typography>
                  ...
                  <Box sx={{ display: 'flex', gap: 0.8, flexWrap: 'wrap', mb: 0.5 }}>
                    {entry.strategy_used && <Chip label={`�Ÿ“Š ${entry.strategy_used}`} size="small" sx={{ height: 20, fontSize: '10px', background: 'rgba(59,130,246,0.1)', color: '#60a5fa', border: '1px solid rgba(59,130,246,0.2)' }} />}
                    {entry.emotional_state && <Chip label={`�Ÿ˜� ${entry.emotional_state}`} size="small" sx={{ height: 20, fontSize: '10px', background: 'rgba(168,85,247,0.1)', color: '#c084fc', border: '1px solid rgba(168,85,247,0.2)' }} />}
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} fullScreen PaperProps={{ sx: { background: 'linear-gradient(180deg, #0f1828 0%, #0a0e1a 100%)', color: 'white' } }}>
        ...
        <DialogContent sx={{ pt: 3 }}>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            ...
            <Box>
              <Typography sx={{ fontSize: '13px', color: 'rgba(255,255,255,0.5)', mb: 1.5 }}>�Ÿ““ Notion (Optional)</Typography>
              <Box sx={{ display: 'flex', gap: 1, mb: 1.5 }}>
                {['template', 'custom'].map(m => <Button key={m} onClick={() => setNotionMode(m as any)} size="small" sx={{ flex: 1, borderRadius: '10px', textTransform: 'none', fontSize: '12px', ...(notionMode === m ? { background: 'linear-gradient(135deg,#a855f7,#ec4899)', color: 'white' } : { border: '1px solid rgba(168,85,247,0.3)', color: '#a855f7' }) }}>{m === 'template' ? '�Ÿ“‹ Template' : '�Ÿ”— Custom'}</Button>)}
              </Box>
              {notionMode === 'template'
