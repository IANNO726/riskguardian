...
let useBranding: any = () => ({ branding: { brand_name: 'RiskGuardian' } });
let TradeBlockBanner: any = () => null;
let TrialBanner: any = () => null;
let MobileNav: any = () => null;
let SupportWidget: any = () => null;
let usePlan: any = () => ({ plan: 'free', refreshPlan: null });
let startCheckout: any = async () => {};

try { const m = require('../hooks/useBranding');   useBranding   = m.useBranding   || useBranding;   } catch {}
try { const m = require('./TradeBlockBanner');      TradeBlockBanner = m.default   || TradeBlockBanner; } catch {}
try { const m = require('./TrialBanner');           TrialBanner   = m.default      || TrialBanner;   } catch {}
try { const m = require('./MobileNav');             MobileNav     = m.default      || MobileNav;     } catch {}
try { const m = require('./SupportWidget');         SupportWidget = m.default      || SupportWidget; } catch {}
try { const m = require('../hooks/usePlan');        usePlan       = m.usePlan      || usePlan;
                                                    startCheckout = m.startCheckout|| startCheckout; } catch {}
...
const PLAN_CONFIG: Record<string, { label: string; color: string; color2: string; glow: string }> = {
  ...
  growth:     { label: 'GROWTH',     color: '#f97316', color2: '#ea580c', glow: '249,115,22'  },
...
const AppShell: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const isPro        = plan === 'pro' || plan === 'growth' || plan === 'enterprise';
  ...
  const switchAccount = async (id: number) => {
    ...
    const token = localStorage.getItem('access_token');
    try {
      await axios.post(`${API}/api/v1/accounts-multi/${id}/set-default`, {}, {
        headers: { Authorization: `Bearer ${token}` },
      });
    ...
    <>
      <style>{`
        ...
        * { font-family: 'Outfit', sans-serif !important; }
        ...
        .plan-badge-shimmer { background: linear-gradient(90deg, transparent 0%, rgba(255,255,255,0.15) 50%, transparent 100%); background-size: 200% auto; animation: shimmer 3s linear infinite; }
      ...
      <Box sx={{ display: 'flex', minHeight: '100vh', background: '#050810', '@media (max-width: 768px)': { flexDirection: 'column' } }}>
        ...
        <Box
          ...
          sx={{
            ...
            '&::before': { content: '""', position: 'absolute', inset: 0, zIndex: 0, backgroundImage: `radial-gradient(ellipse at 50% 0%, rgba(${planCfg.glow}, 0.12) 0%, transparent 60%), radial-gradient(ellipse at 20% 100%, rgba(${planCfg.glow}, 0.06) 0%, transparent 50%)`, pointerEvents: 'none' },
            '&::after': { content: '""', position: 'absolute', left: '-40px', top: `${mouseY - 15}%`, width: '120px', height: '30%', background: `radial-gradient(ellipse, rgba(${planCfg.glow}, 0.08) 0%, transparent 70%)`, pointerEvents: 'none', transition: 'top 0.3s ease', zIndex: 0 },
          }}
        >
          <Box sx={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', height: '100%' }}>
            ...
            <Box sx={{ px: 2, pt: 2.5, pb: 2, display: 'flex', alignItems: 'center', gap: 1.5, minHeight: 72 }}>
              ...
              {isExpanded && (
                <Box sx={{ overflow: 'hidden', flex: 1 }}>
                  <Typography sx={{ fontSize: '16px', fontWeight: 800, letterSpacing: '-0.03em', lineHeight: 1.1, whiteSpace: 'nowrap', background: `linear-gradient(90deg, #fff 30%, ${planCfg.color})`, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
                    {branding?.brand_name || 'RiskGuardian'}
                  </Typography>
                  <Box sx={{ display: 'inline-flex', alignItems: 'center', gap: 0.5, mt: 0.5, px: 1, py: 0.3, borderRadius: '20px', overflow: 'hidden', position: 'relative', background: `linear-gradient(90deg, rgba(${planCfg.glow},0.2), rgba(${planCfg.glow},0.1))`, border: `1px solid rgba(${planCfg.glow}, 0.35)` }}>
                    <Box className="plan-badge-shimmer" sx={{ position: 'absolute', inset: 0, borderRadius: '20px' }} />
                    <Box sx={{ width: 6, height: 6, borderRadius: '50%', background: `rgb(${planCfg.glow})`, boxShadow: `0 0 6px rgb(${planCfg.glow})`, position: 'relative' }} />
            ...
            <Box sx={{ px: 1.5, mb: 2 }}>
              {loading ? (
                ...
                <>
                  <Box onClick={(e) => setAnchorEl(e.currentTarget as HTMLElement)} sx={{ display: 'flex', alignItems: 'center', gap: 1.2, px: isExpanded ? 1.5 : 0.5, py: 1.3, borderRadius: '14px', cursor: 'pointer', position: 'relative', overflow: 'hidden', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)', justifyContent: isExpanded ? 'flex-start' : 'center', transition: 'all 0.25s', '&:hover': { background: 'rgba(255,255,255,0.07)', borderColor: `rgba(${planCfg.glow},0.25)`, transform: 'translateY(-1px)' } }}>
                    <Box sx={{ position: 'relative', flexShrink: 0 }}>
                      <Avatar sx={{ width: 32, height: 32, background: `linear-gradient(135deg, rgba(${planCfg.glow},0.4), rgba(${planCfg.glow},0.1))`, border: `1px solid rgba(${planCfg.glow},0.3)`, fontSize: '13px' }}>
                    ...
                    {isExpanded && (
                      <>
                        <Box sx={{ flex: 1, overflow: 'hidden' }}>
                          <Typography sx={{ fontSize: '12px', fontWeight: 700, color: '#fff', lineHeight: 1, mb: 0.4, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                  ...
                  <Menu anchorEl={anchorEl} open={open} onClose={() => setAnchorEl(null)} PaperProps={{ sx: { mt: 1, minWidth: 290, background: 'linear-gradient(160deg, #141928, #0a0d1a)', border: `1px solid rgba(${planCfg.glow}, 0.15)`, borderRadius: '16px', boxShadow: `0 24px 64px rgba(0,0,0,0.8)`, overflow: 'hidden' } }}>
              ...
              {NAV_ITEMS.map(({ to, label, icon, sub, end, minPlan, color: itemColor, glow: itemGlow }) => {
                ...
                return (
                  <NavLink key={to} to={to} end={end} style={{ textDecoration: 'none' }}>
                    <Box className={`nav-item${active ? ' active-item' : ''}`} sx={{ display: 'flex', alignItems: 'center', gap: isExpanded ? 1.4 : 0, justifyContent: isExpanded ? 'flex-start' : 'center', px: isExpanded ? 1.4 : 0, py: isExpanded ? 1.2 : 1.3, mb: 0.6, borderRadius: '13px', cursor: locked ? 'not-allowed' : 'pointer', position: 'relative', overflow: 'hidden', opacity: locked ? 0.35 : 1, background: active ? `linear-gradient(135deg, rgba(${itemGlow},0.22) 0%, rgba(${itemGlow},0.08) 100%)` : `rgba(${itemGlow},0.03)`, border: active ? `1px solid rgba(${itemGlow},0.4)` : `1px solid rgba(${itemGlow},0.06)`, boxShadow: active ? `0 4px 24px rgba(${itemGlow},0.18)` : 'none', transition: 'all 0.25s cubic-bezier(0.34,1.56,0.64,1)', '&:hover': !locked ? { background: active ? undefined : 'rgba(255,255,255,0.05)', borderColor: active ? undefined : 'rgba(255,255,255,0.09)' } : {}, '&::before': active ? { content: '""', position: 'absolute', left: 0, top: '15%', bottom: '15%', width: '3px', borderRadius: '0 3px 3px 0', background: itemColor, boxShadow: `0 0 12px rgba(${itemGlow},0.8)` } : {} }}>
                      <Box sx={{ width: 38, height: 38, borderRadius: '11px', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: active ? `linear-gradient(135deg, rgba(${itemGlow},0.3), rgba(${itemGlow},0.1))` : `rgba(${itemGlow},0.1)`, border: active ? `1px solid rgba(${itemGlow},0.45)` : `1px solid rgba(${itemGlow},0.2)`, transition: 'all 0.25s', boxShadow: active ? `0 0 20px rgba(${itemGlow},0.35)` : `0 0 8px rgba(${itemGlow},0.1)` }}>
                      ...
                      {isExpanded && (
                        <Box sx={{ flex: 1, overflow: 'hidden' }}>
                          <Typography sx={{ fontSize: '14px', fontWeight: active ? 800 : 600, color: active ? '#fff' : 'rgba(255,255,255,0.92)', lineHeight: 1, mb: 0.3, letterSpacing: '-0.01em' }}>{label}</Typography>
                ...
                return (
                  <NavLink to="/app/journal" style={{ textDecoration: 'none' }}>
                    <Box className={`nav-item${active ? ' active-item' : ''}`} sx={{ display: 'flex', alignItems: 'center', gap: isExpanded ? 1.4 : 0, justifyContent: isExpanded ? 'flex-start' : 'center', px: isExpanded ? 1.4 : 0, py: isExpanded ? 1.2 : 1.3, mb: 0.6, borderRadius: '13px', cursor: 'pointer', position: 'relative', overflow: 'hidden', background: active ? `linear-gradient(135deg, rgba(${planCfg.glow},0.22), rgba(${planCfg.glow},0.08))` : `rgba(${planCfg.glow},0.03)`, border: active ? `1px solid rgba(${planCfg.glow},0.4)` : `1px solid rgba(${planCfg.glow},0.07)`, transition: 'all 0.25s cubic-bezier(0.34,1.56,0.64,1)', '&:hover': { background: active ? undefined : 'rgba(255,255,255,0.05)' }, '&::before': active ? { content: '""', position: 'absolute', left: 0, top: '15%', bottom: '15%', width: '3px', borderRadius: '0 3px 3px 0', background: `linear-gradient(180deg,${planCfg.color},${planCfg.color2})`, boxShadow: `0 0 12px rgba(${planCfg.glow},0.8)` } : {} }}>
                      ...
                      {isExpanded && active && <Box sx={{ width: 7, height: 7, borderRadius: '50%', background: planCfg.color, flexShrink: 0 }} />}
                    </Box>
                  </NavLink>
                );
              })()}
            ...
            <Box sx={{ px: 1.5, pb: 2.5, pt: 1.5 }}>
              ...
              {isExpanded && (plan === 'free' || plan === 'starter') && (
                <Box onClick={() => !upgradeLoading && startCheckout(plan === 'free' ? 'starter' : 'pro', setUpgradeLoading)} sx={{ mb: 1.5, p: 1.5, borderRadius: '14px', cursor: upgradeLoading ? 'not-allowed' : 'pointer', background: `linear-gradient(135deg, rgba(${planCfg.glow},0.15) 0%, rgba(${planCfg.glow},0.05) 100%)`, border: `1px solid rgba(${planCfg.glow},0.3)`, textAlign: 'center', transition: 'all 0.25s', position: 'relative', overflow: 'hidden', '&:hover': !upgradeLoading ? { transform: 'translateY(-2px)' } : {} }}>
                  {upgradeLoading ? (
