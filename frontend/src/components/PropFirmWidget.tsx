...
const PropFirmWidget: React.FC = () => {
  ...
  if (!features.prop_firm_profiles) {
    return (
      <Box sx={{
        ...
        border: '1px solid rgba(255,255,255,0.06)', opacity: 0.5, cursor: 'not-allowed',
  ...
  if (active.active && active.profile) {
    ...
    return (
      <Box sx={{
        ...
        position: 'relative', overflow: 'hidden',
        '&::before': { content: '""', position: 'absolute', top: 0, left: 0, right: 0, height: '2px', background: `linear-gradient(90deg, transparent, ${p.color}, transparent)` }
      }}>
  ...
  return (
    <>
      <Box sx={{
        p: 2.5, borderRadius: '16px', background: 'rgba(255,255,255,0.03)',
            ...
            {Object.entries(presets).map(([key, preset]) => (
              <Grid item xs={12} sm={6} key={key}>
                <Box onClick={() => activate(key)} sx={{
                  ...
                  transition: 'all 0.2s',
                  '&:hover': { transform: 'translateY(-4px)', boxShadow: `0 8px 24px ${preset.color}22`, border: `1px solid ${preset.color}66` }
                }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 1.5 }}>
