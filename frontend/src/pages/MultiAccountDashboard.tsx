...
const DERIV_STEPS = [
  { icon:"�ŸŒ�", title:"Open Deriv", desc:'Go to app.deriv.com and make sure you\'re logged in', action:{ label:"Open Deriv �†’", url:"https://app.deriv.com/account/api-token" } },
  { icon:"�Ÿ‘�", title:"Go to API Token", desc:'Click your profile icon �†’ Settings �†’ "Security & privacy" �†’ API Token' },
  { icon:"�œ�️", title:"Create a token", desc:'Name it "RiskGuardian", enable Read + Trading information scopes, click Create' },
  { icon:"�Ÿ“‹", title:"Copy & paste here", desc:"Copy the token and paste it in the field below �€” not your MT5 password" },
];
...
function AddAccountModal({ open, onClose, onAdded }: {
  ...
  const friendlyError = (detail: string) => {
    ...
    if (d.includes("invalid account") || d.includes("authorization failed") || d.includes("no connection"))
      return "�Œ Connection failed �€” check your account number, password and server name.";
    if (d.includes("token") || d.includes("api"))
      return "�Œ Invalid API token �€” make sure it has Read + Trading information scopes enabled.";
    if (d.includes("password"))  return "�Œ Incorrect password �€” please double-check.";
    if (d.includes("server"))    return "�Œ Server not found. Example: Deriv-Demo or Deriv-Server.";
    if (d.includes("limit reached") || d.includes("403"))
      return "�š� Account limit reached for your plan �€” upgrade to add more.";
    if (d.includes("already exists")) return "�š�️ This account is already connected.";
    return `�Œ ${detail || "Failed to connect. Please check your details and try again."}`;
  };
  ...
  const submit = async () => {
    ...
    try {
      ...
      setStep("form");
      setError("�Œ Network error �€” make sure the backend is running.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <Box sx={{
        ...
        {step === "success" ? (
          <Box sx={{ textAlign:"center", py:3 }}>
            <Box sx={{ width:72,height:72,borderRadius:"50%",mx:"auto",mb:2,background:"rgba(34,197,94,0.12)",border:"1px solid rgba(34,197,94,0.3)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"30px" }}>�œ…</Box>
          ...
          <>
            ...
            <Box>
              <Box sx={{ display:"flex",alignItems:"center",gap:"8px",mb:"6px",flexWrap:"wrap" }}>
                ...
                {isDerivMode && <Box sx={{ fontSize:"9px",fontWeight:800,px:"7px",py:"2px",borderRadius:"5px",color:"#fbbf24",background:"rgba(251,191,36,0.12)",border:"1px solid rgba(251,191,36,0.2)",letterSpacing:"0.5px" }}>�š� NOT YOUR MT5 PASSWORD</Box>}
              </Box>
              <Box sx={{ position:"relative",mb:"8px" }}>
                ...
                <Box onClick={()=>setShowPwd(v=>!v)} sx={{ position:"absolute",right:"12px",top:"50%",transform:"translateY(-50%)",cursor:"pointer",fontSize:"15px",color:showPwd?"#38bdf8":"rgba(255,255,255,0.28)","&:hover":{color:"rgba(255,255,255,0.7)"},transition:"color 0.15s" }}>{showPwd?"�Ÿ™ˆ":"�Ÿ‘�"}</Box>
              </Box>
              {isDerivMode && (
                <Box sx={{ display:"flex",alignItems:"flex-start",gap:"8px",p:"10px 12px",mb:"12px",background:"rgba(251,191,36,0.05)",border:"1px dashed rgba(251,191,36,0.2)",borderRadius:"10px" }}>
                  <Typography sx={{ fontSize:"16px",lineHeight:1 }}>�Ÿ”‘</Typography>
            ...
            <Box sx={{ display:"flex",gap:"10px",mt:"4px" }}>
              ...
              <Box onClick={!loading?submit:undefined} sx={{ flex:2,py:"11px",borderRadius:"12px",textAlign:"center",cursor:loading?"not-allowed":"pointer",fontSize:"13.5px",fontWeight:700,background:loading?(isDerivMode?"rgba(251,191,36,0.25)":"rgba(56,189,248,0.25)"):isDerivMode?"linear-gradient(135deg,#d97706,#f59e0b,#fbbf24)":"linear-gradient(135deg,#0ea5e9,#6366f1)",color:isDerivMode?"#000":"white",display:"flex",alignItems:"center",justifyContent:"center",gap:"8px",opacity:loading?0.7:1,boxShadow:loading?"none":isDerivMode?"0 4px 20px rgba(251,191,36,0.3)":"0 4px 20px rgba(56,189,248,0.25)","&:hover":!loading?{transform:"translateY(-1px)",boxShadow:isDerivMode?"0 6px 24px rgba(251,191,36,0.4)":"0 6px 24px rgba(56,189,248,0.35)"}:{},transition:"all 0.15s" }}>
                {loading?<><CircularProgress size={13} sx={{color:isDerivMode?"#000":"white"}}/>{isDerivMode?"Connecting via WebSocket�€�":"Verifying�€�"}</>:isDerivMode?"�š� Connect Deriv Account �†’":"�Ÿ”’ Connect & Verify �†’"}
              </Box>
            </Box>
            <Typography sx={{ textAlign:"center",mt:"14px",fontSize:"11px",color:"rgba(255,255,255,0.2)" }}>�Ÿ”� Encrypted end-to-end · Never stored in plain text · Read-only access</Typography>
          </>
        )}
      </Box>
    </Modal>
    ...
    <Box sx={{
      ...
      <Box sx={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start",mb:"14px" }}>
        ...
        <Box sx={{ display:"flex",flexDirection:"column",alignItems:"flex-end",gap:"5px",ml:1 }}>
          ...
          {acc.mt5_connected===false && (
            <Box sx={{ fontSize:"9px",fontWeight:700,px:"7px",py:"2px",borderRadius:"6px",color:"#f59e0b",background:"rgba(245,158,11,0.09)",border:"1px solid rgba(245,158,11,0.25)",whiteSpace:"nowrap" }}>�š� Offline</Box>
          )}
        </Box>
      </Box>
      ...
      <Box sx={{ display:"flex",gap:"7px",mb:"14px" }}>
        ...
        <Box sx={{ flex:1,textAlign:"center",borderRadius:"9px",py:"8px",background:acc.risk_locked?"rgba(168,85,247,0.12)":"rgba(0,0,0,0.18)" }}>
          <Typography sx={{ fontSize:"12px",fontWeight:800,color:acc.risk_locked?"#a855f7":"#22c55e" }}>{acc.risk_locked?"�Ÿ”’ ON":"�œ“ OFF"}</Typography>
    ...
    <Box sx={{ minHeight:"100vh",p:{xs:2,md:"28px 32px"},color:"white",background:"radial-gradient(ellipse at 10% 0%,rgba(56,189,248,0.04),transparent 55%),radial-gradient(ellipse at 90% 100%,rgba(168,85,247,0.04),transparent 55%),#08101e" }}>
      ...
      <Box sx={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start",mb:"28px",flexWrap:"wrap",gap:2 }}>
        <Box>
          ...
          <Box sx={{ display:"flex",alignItems:"center",gap:"10px",mt:"5px",flexWrap:"wrap" }}>
            ...
            {atRiskCount>0 && <Typography sx={{ fontSize:"12px",color:"#f59e0b",fontWeight:600 }}>�š� {atRiskCount} at risk</Typography>}
            {lockedCount>0 && <Typography sx={{ fontSize:"12px",color:"#a855f7",fontWeight:600 }}>�Ÿ”’ {lockedCount} locked</Typography>}
          </Box>
        </Box>
        <Box sx={{ display:"flex",gap:"10px",alignItems:"center",flexWrap:"wrap" }}>
          {planInfo && (
            <Box sx={{ px:"14px",py:"8px",borderRadius:"10px",fontSize:"12px",fontWeight:700,background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.08)",color:"rgba(255,255,255,0.45)" }}>
              {planInfo.used}/{planInfo.max===999?"�ˆž":planInfo.max} · {(planInfo.plan||"free").toUpperCase()}
            </Box>
          )}
          <Box onClick={handleRefresh} sx={{ px:"14px",py:"8px",borderRadius:"10px",cursor:refreshing?"default":"pointer",fontSize:"12.5px",fontWeight:700,display:"flex",alignItems:"center",gap:"6px",background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.08)",color:refreshing?"#38bdf8":"rgba(255,255,255,0.4)","&:hover":!refreshing?{background:"rgba(255,255,255,0.07)",color:"white"}:{},transition:"all 0.15s" }}>
            {refreshing?<><CircularProgress size={11} sx={{color:"#38bdf8"}}/> Syncing�€�</>:"�Ÿ� Refresh"}
          </Box>
          {planInfo?.can_add ? (
            ...
            <Box onClick={()=>(window.location.hash="/app/settings")} sx={{ px:"16px",py:"8px",borderRadius:"10px",cursor:"pointer",fontSize:"13px",fontWeight:700,background:"rgba(168,85,247,0.09)",border:"1px solid rgba(168,85,247,0.25)",color:"#a855f7","&:hover":{background:"rgba(168,85,247,0.15)"},transition:"all 0.15s" }}>�š� Upgrade</Box>
          )}
        </Box>
      </Box>
      ...
      {accounts.length===0 ? (
        <Box sx={{ ...card,p:"52px 40px",textAlign:"center",border:"1px dashed rgba(56,189,248,0.15)",background:"rgba(56,189,248,0.02)" }}>
          <Typography sx={{ fontSize:"40px",mb:"14px" }}>�Ÿ–�️</Typography>
          ...
          {planInfo?.can_add ? (
            ...
            <Box onClick={()=>(window.location.hash="/app/settings")} sx={{ display:"inline-flex",px:"28px",py:"12px",borderRadius:"12px",cursor:"pointer",fontSize:"13.5px",fontWeight:700,background:"rgba(168,85,247,0.12)",border:"1px solid rgba(168,85,247,0.3)",color:"#a855f7" }}>�š� Upgrade to add accounts</Box>
          )}
        </Box>
      ) : (
      ...
      {alerts.length>0 && (
        <Box sx={{ ...card,p:"20px 24px",mb:"20px" }}>
          <Typography sx={{ fontSize:"11px",fontWeight:700,color:"rgba(255,255,255,0.35)",textTransform:"uppercase",letterSpacing:"0.1em",mb:"14px" }}>�Ÿ”” Active Alerts</Typography>
          {alerts.map((a,i)=>(
      ...
      <Modal open={!!deleteConfirm} onClose={()=>setDeleteConfirm(null)}>
        <Box sx={{ position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",width:{xs:"88%",sm:390},background:"#0d1625",border:"1px solid rgba(239,68,68,0.2)",borderRadius:"18px",p:"28px",boxShadow:"0 30px 80px rgba(0,0,0,0.7)" }}>
          <Typography sx={{ fontSize:"22px",mb:"10px" }}>�Ÿ—‘️</Typography>
