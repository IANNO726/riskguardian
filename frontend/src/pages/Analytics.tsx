...
const LockHistorySection: React.FC = () => {
  ...
  if (!data || data.total_locks === 0) return (
    <Box sx={{ p: 4, borderRadius: '24px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', textAlign: 'center' }}>
      <Typography sx={{ fontSize: '40px', opacity: 0.2, mb: 1.5 }}>�Ÿ”’</Typography>
  ...
  return (
    <Box>
      ...
      <Grid container spacing={2} sx={{ mb: 3 }}>
        {[
          ...
          },
          {
            ...
            icon: data.locks_this_week === 0 ? '�œ…' : '�Ÿ“…',
          },
          {
            label: 'Avg Duration',
            value: avgDurStr,
      ...
      <Grid container spacing={3} sx={{ mb: 3 }}>
        ...
        <Grid item xs={12} md={7}>
          <Box sx={{ p: 3, borderRadius: '20px', background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.07)', height: '100%' }}>
            <Typography sx={{ fontSize: '14px', fontWeight: 700, color: 'white', mb: 2 }}>�Ÿ”’ Locks Per Week (Last 8 Weeks)</Typography>
        ...
        <Grid item xs={12} md={5}>
          <Box sx={{ p: 3, borderRadius: '20px', background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.07)', height: '100%' }}>
            <Typography sx={{ fontSize: '14px', fontWeight: 700, color: 'white', mb: 2 }}>�Ÿ�� Lock Triggers Breakdown</Typography>
            {barData.map(item => {
      ...
      <Box sx={{ p: 3, borderRadius: '20px', background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.07)' }}>
        <Typography sx={{ fontSize: '14px', fontWeight: 700, color: 'white', mb: 2 }}>�Ÿ“‹ Recent Lock Events</Typography>
              ...
              <Box key={event.id} sx={{
                ...
                <Box sx={{ display: 'flex', gap: 1, flexShrink: 0, flexWrap: 'wrap' }}>
                  ...
                  {event.triggered_by === 'auto' && (
                    <Chip label="�Ÿ�– Auto" size="small" sx={{ height: 24, fontSize: '11px', fontWeight: 600, background: 'rgba(168,85,247,0.15)', color: '#a855f7', border: '1px solid rgba(168,85,247,0.3)' }} />
                  )}
  ...
  return (
    <Box sx={{ p: 3, borderRadius: '20px', background: enabled ? 'rgba(239,68,68,0.06)' : 'rgba(255,255,255,0.025)', border: `1px solid ${enabled ? 'rgba(239,68,68,0.2)' : 'rgba(255,255,255,0.07)'}`, mt: 3 }}>
      ...
      {enabled && (
        <Box sx={{ mb: 2, p: 2, borderRadius: '12px', background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.2)' }}>
          <Typography sx={{ fontSize: '13px', color: 'rgba(255,255,255,0.7)', lineHeight: 1.7 }}>
            �š� When your daily loss reaches <strong style={{ color: '#ef4444' }}>{threshold}% of balance</strong>, Risk Lock activates automatically for <strong style={{ color: '#ef4444' }}>{minutes >= 60 ? `${minutes/60}h` : `${minutes}m`}</strong> and logs the event to your journal.
          </Typography>
        </Box>
      )}
      ...
      <Button onClick={save} disabled={saving}
        ...
        {saving ? 'Saving�€�' : saved ? '�œ… Saved!' : 'Save Settings'}
      </Button>
    </Box>
  );
};
        ...
        <Grid item xs={12} sm={6} xl={3}>
          <Box sx={{ ...cardBase, background: 'linear-gradient(135deg,rgba(56,189,248,0.1),rgba(56,189,248,0.03))', border: '1px solid rgba(56,189,248,0.2)', '&:hover': { ...cardBase['&:hover'], boxShadow: '0 20px 50px rgba(56,189,248,0.18)' }, '&::before': { ...cardBase['&::before'], background: 'linear-gradient(90deg,transparent,#38bdf8,transparent)' } }}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 2 }}>
              ...
              <Box sx={{ width: 42, height: 42, borderRadius: '12px', background: 'rgba(56,189,248,0.15)', border: '1px solid rgba(56,189,248,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px', flexShrink: 0 }}>�Ÿ’�</Box>
            </Box>
        ...
        <Grid item xs={12} sm={6} xl={3}>
          <Box sx={{ ...cardBase, background: data.return_pct >= 0 ? 'linear-gradient(135deg,rgba(34,197,94,0.1),rgba(34,197,94,0.03))' : 'linear-gradient(135deg,rgba(239,68,68,0.1),rgba(239,68,68,0.03))', border: `1px solid ${data.return_pct >= 0 ? 'rgba(34,197,94,0.2)' : 'rgba(239,68,68,0.2)'}`, '&:hover': { ...cardBase['&:hover'], boxShadow: `0 20px 50px ${data.return_pct >= 0 ? 'rgba(34,197,94,0.18)' : 'rgba(239,68,68,0.18)'}` }, '&::before': { ...cardBase['&::before'], background: `linear-gradient(90deg,transparent,${data.return_pct >= 0 ? '#22c55e' : '#ef4444'},transparent)` } }}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 2 }}>
              ...
              <Box sx={{ width: 42, height: 42, borderRadius: '12px', background: data.return_pct >= 0 ? 'rgba(34,197,94,0.15)' : 'rgba(239,68,68,0.15)', border: `1px solid ${data.return_pct >= 0 ? 'rgba(34,197,94,0.3)' : 'rgba(239,68,68,0.3)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px', flexShrink: 0 }}>{data.return_pct >= 0 ? '�Ÿ“ˆ' : '�Ÿ“‰'}</Box>
            </Box>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <Box sx={{ flex: 1, minWidth: 0 }}>
                ...
                <Box sx={{ mt: 2, pt: 2, borderTop: '1px solid rgba(255,255,255,0.07)' }}>
                  ...
                  <Typography sx={{ fontSize: '10px', color: pfColor, fontWeight: 600 }}>{data.profit_factor >= 2 ? '�œ… Excellent' : data.profit_factor >= 1 ? '�š�️ Acceptable' : '�Œ Below 1.0'}</Typography>
                </Box>
              </Box>
              <Box sx={{ width: 42, height: 42, borderRadius: '12px', background: 'rgba(239,68,68,0.15)', border: '1px solid rgba(239,68,68,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px', flexShrink: 0 }}>�š�️</Box>
            </Box>
          </Box>
        </Grid>
      </Grid>
      ...
      <Box sx={{ borderRadius: '24px', background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.07)', mb: { xs: 3, md: 4 }, overflow: 'hidden' }}>
        <Box sx={{ px: { xs: 2, md: 4 }, pt: { xs: 2, md: 3 }, pb: 2, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 2, borderBottom: '1px solid rgba(255,255,255,0.06)', background: isEquity ? 'rgba(34,197,94,0.03)' : 'rgba(239,68,68,0.03)' }}>
          <Box>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
              <Typography sx={{ fontSize: { xs: '15px', md: '17px' }, fontWeight: 700, color: 'white' }}>
                {isEquity ? '�Ÿ“ˆ Equity Curve' : '�Ÿ“‰ Drawdown Chart'}
              </Typography>
          ...
          <ToggleButtonGroup value={mode} exclusive onChange={(e, val) => val && setMode(val)} size="small"
            ...
            <ToggleButton value="equity">�Ÿ“ˆ Equity</ToggleButton>
            <ToggleButton value="drawdown">�Ÿ“‰ Drawdown</ToggleButton>
          </ToggleButtonGroup>
        </Box>
      ...
      <Grid container spacing={{ xs: 2, md: 3 }} sx={{ mb: { xs: 3, md: 4 } }}>
        <Grid item xs={12} lg={7}>
          <Box sx={{ p: { xs: 2.5, md: 3.5 }, borderRadius: '24px', background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.07)', height: '100%' }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 3 }}>
              <Box sx={{ width: 36, height: 36, borderRadius: '10px', background: 'rgba(56,189,248,0.15)', border: '1px solid rgba(56,189,248,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '17px' }}>�Ÿ“‹</Box>
              <Box>
            ...
            <Grid container spacing={1.5}>
              {[
                { label: 'Total Profit',  value: `+$${data.total_profit.toFixed(2)}`,                                  color: '#22c55e', bg: 'rgba(34,197,94,0.08)',   border: 'rgba(34,197,94,0.2)',  icon: '�Ÿ’š' },
                { label: 'Total Loss',    value: `-$${data.total_loss.toFixed(2)}`,                                    color: '#ef4444', bg: 'rgba(239,68,68,0.08)',  border: 'rgba(239,68,68,0.2)', icon: '�Ÿ”�' },
                { label: 'Net P&L',       value: `${data.net_profit >= 0 ? '+' : ''}$${data.net_profit.toFixed(2)}`,  color: data.net_profit >= 0 ? '#22c55e' : '#ef4444', bg: data.net_profit >= 0 ? 'rgba(34,197,94,0.08)' : 'rgba(239,68,68,0.08)', border: data.net_profit >= 0 ? 'rgba(34,197,94,0.2)' : 'rgba(239,68,68,0.2)', icon: '�Ÿ’�' },
                ...
                { label: 'Profit Factor', value: data.profit_factor.toFixed(2),                                        color: pfColor,   bg: 'rgba(255,255,255,0.04)', border: 'rgba(255,255,255,0.1)', icon: '�š–️' },
                { label: 'Best Trade',    value: `+$${data.best_trade.toFixed(2)}`,                                    color: '#38bdf8', bg: 'rgba(56,189,248,0.08)',  border: 'rgba(56,189,248,0.2)', icon: '�Ÿ�†' },
                { label: 'Worst Trade',   value: `$${data.worst_trade.toFixed(2)}`,                                    color: '#f59e0b', bg: 'rgba(251,191,36,0.08)',  border: 'rgba(251,191,36,0.2)', icon: '�š�' },
                { label: 'Total Trades',  value: `${data.total_trades}`,                                               color: '#a855f7', bg: 'rgba(168,85,247,0.08)',  border: 'rgba(168,85,247,0.2)', icon: '�Ÿ“Š' },
              ].map((row) => (
            ...
            <Box sx={{ mt: 2.5, p: 2.5, borderRadius: '16px', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1.5, flexWrap: 'wrap', gap: 1 }}>
                <Typography sx={{ fontSize: '12px', fontWeight: 700, color: '#22c55e' }}>�Ÿ�† Wins: {data.winning_trades} ({winPct.toFixed(1)}%)</Typography>
                <Typography sx={{ fontSize: '12px', fontWeight: 700, color: '#ef4444' }}>�Œ Losses: {data.losing_trades} ({(100-winPct).toFixed(1)}%)</Typography>
              </Box>
        ...
        <Grid item xs={12} lg={5}>
          <Box sx={{ p: { xs: 2.5, md: 3 }, borderRadius: '24px', background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.07)', height: '100%' }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 2 }}>
              <Box sx={{ width: 36, height: 36, borderRadius: '10px', background: 'rgba(168,85,247,0.15)', border: '1px solid rgba(168,85,247,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '17px' }}>�Ÿ“…</Box>
              <Box>
