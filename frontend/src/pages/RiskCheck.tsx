...
function ActionBadge({ action }: { action: string }) {
  const map: any = {
    PROCEED: { bg:"rgba(34,197,94,0.12)",  border:"rgba(34,197,94,0.3)",  color:"#22c55e", icon:"�œ…", label:"PROCEED"     },
    REDUCE:  { bg:"rgba(245,158,11,0.12)", border:"rgba(245,158,11,0.3)", color:"#f59e0b", icon:"�š�️", label:"REDUCE LOT"  },
    AVOID:   { bg:"rgba(239,68,68,0.12)",  border:"rgba(239,68,68,0.3)",  color:"#ef4444", icon:"�Ÿš�", label:"AVOID TRADE" },
  };
...
function SymbolPicker({ value, onChange }: { value: string; onChange: (s: string) => void }) {
  ...
  const tabs = [
    { k:"vol",   label:"�Ÿ“Š Vol",        c:"168,85,247" },
    { k:"crash", label:"�Ÿ’� Crash/Boom", c:"239,68,68"  },
    { k:"step",  label:"�Ÿ�œ Step",       c:"245,158,11" },
    { k:"other", label:"�ŸŽ� Other",      c:"34,197,94"  },
    { k:"forex", label:"�Ÿ’� Forex",      c:"56,189,248" },
  ] as const;
  ...
  return (
    <Box mb={1.5}>
      ...
      <Box sx={{ position:"relative" }}>
        ...
        {isCustom && (
          <Box sx={{ position:"absolute", right:"10px", top:"50%", transform:"translateY(-50%)", px:"8px", py:"2px", borderRadius:"6px", background:"rgba(245,158,11,0.15)", border:"1px solid rgba(245,158,11,0.35)", display:"flex", alignItems:"center", gap:"4px" }}>
            <Typography sx={{ fontSize:"9px", fontWeight:800, color:"#f59e0b", textTransform:"uppercase", letterSpacing:"0.05em" }}>�œ�️ Custom</Typography>
          </Box>
        )}
      </Box>
      {isCustom && (
        <Box sx={{ mt:"6px", p:"7px 12px", borderRadius:"8px", background:"rgba(245,158,11,0.05)", border:"1px solid rgba(245,158,11,0.15)" }}>
          <Typography sx={{ fontSize:"11px", color:"rgba(245,158,11,0.8)" }}>
            �œ… Custom symbol detected �€” formula will be auto-detected from the name.
          </Typography>
        </Box>
      )}
      ...
      <Typography sx={{ fontSize:"10px", color:"rgba(255,255,255,0.2)", mt:"6px", fontStyle:"italic" }}>
        �Ÿ’� Not in the list? Just type the symbol name above �€” it will compute automatically.
      </Typography>
    </Box>
  );
}
      ...
      <Box sx={{ display:"flex", alignItems:"center", justifyContent:"space-between", mb:"6px" }}>
        ...
        <Box onClick={onToggle} sx={{ display:"flex", alignItems:"center", gap:"6px", cursor:"pointer", px:"8px", py:"3px", borderRadius:"8px", background:useCustom?"rgba(245,158,11,0.1)":"rgba(255,255,255,0.04)", border:useCustom?"1px solid rgba(245,158,11,0.3)":"1px solid rgba(255,255,255,0.08)", transition:"all 0.15s" }}>
          <Box sx={{ width:12, height:12, borderRadius:"3px", background:useCustom?"#f59e0b":"rgba(255,255,255,0.1)", display:"flex", alignItems:"center", justifyContent:"center" }}>
            {useCustom && <Typography sx={{ fontSize:"9px", color:"#000", fontWeight:900, lineHeight:1 }}>�œ“</Typography>}
          </Box>
...
function RiskCheckTab({ liveBalance }: { liveBalance: number }) {
  ...
  const copyResult = (r: EvalResult) => {
    const lines = [
      `�Ÿ“Š RiskGuardian �€” Trade Risk Report`,
      `Symbol:      ${r.symbol}`,
      ...
      r.prop_check ? `Prop (${r.prop_check.firm}): ${r.prop_check.passed ? "PASSED �œ…" : "FAILED �Œ"}` : "",
    ].filter(Boolean).join("\n");
  ...
  return (
    <Grid container spacing={3}>
      ...
      <Grid item xs={12} md={5}>
        <Box sx={{ ...card(), p:"24px" }}>
          <Typography sx={{ fontSize:"14px", fontWeight:800, mb:"18px", color:"rgba(255,255,255,0.8)" }}>�Ÿ“‹ Trade Parameters</Typography>

          <Box sx={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"8px", mb:"18px" }}>
            {[
              { k:"lot",  icon:"�Ÿ“�", label:"Lot �†’ Risk",  sub:"Enter lot, see risk",    c:"56,189,248" },
              { k:"risk", icon:"�ŸŽ�", label:"Risk �†’ Lot",  sub:"Enter risk %, get lot",  c:"34,197,94"  },
            ].map(t => (
          ...
          <Box onClick={!loading?evaluate:undefined} sx={{ py:"13px", borderRadius:"13px", textAlign:"center", cursor:loading?"not-allowed":"pointer", fontWeight:800, fontSize:"14px", background:loading?"rgba(56,189,248,0.2)":"linear-gradient(135deg,#0ea5e9,#6366f1)", color:"white", display:"flex", alignItems:"center", justifyContent:"center", gap:1, opacity:loading?0.7:1, boxShadow:loading?"none":"0 6px 24px rgba(56,189,248,0.25)", transition:"all 0.15s", "&:hover":!loading?{transform:"translateY(-1px)"}:{} }}>
            {loading ? <><CircularProgress size={14} sx={{color:"white"}}/> Analysing�€�</> : "�Ÿ”� Evaluate Trade Risk �†’"}
          </Box>
        </Box>
      </Grid>
      ...
      <Grid item xs={12} md={7}>
        {!result && !loading && (
          <Box sx={{ ...card(), p:"52px 40px", textAlign:"center", border:"1px dashed rgba(56,189,248,0.1)" }}>
            <Typography sx={{ fontSize:"48px", mb:2, opacity:0.12 }}>�ŸŽ�</Typography>
          ...
          <Box sx={{ display:"flex", flexDirection:"column", gap:2 }}>
            ...
            {inputMode==="risk" && (
              <Box sx={{ ...card(), p:"20px 24px", background:"linear-gradient(135deg,rgba(34,197,94,0.1),rgba(56,189,248,0.05))", border:"1px solid rgba(34,197,94,0.3)" }}>
                <Typography sx={{ fontSize:"11px", color:"rgba(34,197,94,0.7)", fontWeight:700, textTransform:"uppercase", letterSpacing:"0.08em", mb:"10px" }}>�ŸŽ� Risk-First �€” Computed Lot Size</Typography>
              ...
              <Grid container alignItems="center" spacing={2}>
                <Grid item xs={12} sm={5}>
                  ...
                  <Typography sx={{ textAlign:"center", fontSize:"12px", fontWeight:700, color:result.safety_score.color, textTransform:"uppercase", letterSpacing:"0.08em", mt:1 }}>
                    {result.safety_score.zone==="safe"?"�œ… Safe to Trade":result.safety_score.zone==="caution"?"�š�️ Caution":"�Ÿš� Dangerous"}
                  </Typography>
                </Grid>
                <Grid item xs={12} sm={7}>
                  ...
                  <Box sx={{ mt:1, p:"7px 10px", borderRadius:"8px", background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.06)" }}>
                    <Typography sx={{ fontSize:"10px", color:"rgba(255,255,255,0.25)", fontFamily:'"Roboto Mono",monospace' }}>�Ÿ“� {result.metrics.formula_used}</Typography>
                  </Box>
                </Grid>
              </Grid>
            </Box>
            ...
            <Grid container spacing={1.5}>
              {[
                { label:"Dollar Risk",      value:`$${result.metrics.dollar_risk.toFixed(2)}`,   color:result.metrics.risk_pct>2?"#ef4444":result.metrics.risk_pct>1?"#f59e0b":"#22c55e", icon:"�Ÿ›‘" },
                { label:"Dollar Profit",    value:`$${result.metrics.dollar_profit.toFixed(2)}`, color:"#22c55e", icon:"�Ÿ’�" },
                { label:"Risk %",           value:`${result.metrics.risk_pct.toFixed(4)}%`,     color:result.metrics.risk_pct>2?"#ef4444":result.metrics.risk_pct>1?"#f59e0b":"#22c55e", icon:"�Ÿ“Š" },
                { label:"Risk:Reward",      value:`1:${result.metrics.rr_ratio.toFixed(2)}`,    color:result.metrics.rr_ratio>=2?"#22c55e":result.metrics.rr_ratio>=1?"#f59e0b":"#ef4444", icon:"�š–️" },
                { label:"Max Lot (1%)",     value:String(result.metrics.max_lot_for_1pct),      color:"#a855f7", icon:"�ŸŽ�" },
                { label:"Balance",          value:`$${result.metrics.balance_used.toLocaleString("en",{maximumFractionDigits:0})}`, color:"#38bdf8", icon:"�Ÿ��" },
            ...
            {result.prop_check && (
              <Box sx={{ ...card(), p:"20px 24px", border:result.prop_check.passed?"1px solid rgba(34,197,94,0.2)":"1px solid rgba(239,68,68,0.2)" }}>
                <Box sx={{ display:"flex", alignItems:"center", gap:2, mb:"12px" }}>
                  <Typography sx={{ fontSize:"14px", fontWeight:800 }}>{result.prop_check.passed?"�œ…":"�Œ"} {result.prop_check.firm}</Typography>
                ...
                {result.prop_check.violations.map((v,i) => <Box key={i} sx={{ display:"flex", gap:1.5, mb:1, p:"10px 12px", borderRadius:"10px", background:"rgba(239,68,68,0.07)", border:"1px solid rgba(239,68,68,0.15)" }}><Typography>�Ÿš�</Typography><Typography sx={{ fontSize:"12.5px", color:"#fca5a5" }}>{v}</Typography></Box>)}
                {result.prop_check.warnings.map((w,i)  => <Box key={i} sx={{ display:"flex", gap:1.5, mb:1, p:"10px 12px", borderRadius:"10px", background:"rgba(245,158,11,0.07)",  border:"1px solid rgba(245,158,11,0.15)"  }}><Typography>�š�️</Typography><Typography sx={{ fontSize:"12.5px", color:"#fde68a" }}>{w}</Typography></Box>)}
            ...
            <Box sx={{ ...card(), p:"20px 24px" }}>
              ...
              {result.checklist.map((item,i) => (
                <Box key={i} sx={{ display:"flex", alignItems:"flex-start", gap:1.5, py:"9px", borderBottom:i<result.checklist.length-1?"1px solid rgba(255,255,255,0.05)":"none" }}>
                  <Box sx={{ width:20, height:20, borderRadius:"6px", flexShrink:0, display:"flex", alignItems:"center", justifyContent:"center", fontSize:"12px", background:item.passed?"rgba(34,197,94,0.12)":"rgba(239,68,68,0.12)", border:`1px solid ${item.passed?"rgba(34,197,94,0.3)":"rgba(239,68,68,0.3)"}` }}>{item.passed?"�œ“":"�œ—"}</Box>
      ...
      <Box sx={{ ...card(), p:"22px", mb:2, borderLeft:"3px solid #38bdf8" }}>
        ...
        {committedPreview !== null && committedPreview > 0 && (
          <Box sx={{ mt:"12px", p:"9px 14px", borderRadius:"9px", background:"rgba(56,189,248,0.05)", border:"1px solid rgba(56,189,248,0.15)", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
            <Typography sx={{ fontSize:"12px", color:"rgba(56,189,248,0.7)" }}>�Ÿ’™ Risk committed by Trade 1</Typography>
        ...
        <Grid item xs={12} sm={7}>
          <Box sx={{ ...card(), p:"18px 20px", height:"100%" }}>
            <Box sx={{ display:"flex", alignItems:"center", justifyContent:"space-between", mb:"12px" }}>
              <Typography sx={{ fontSize:"13px", fontWeight:800, color:"rgba(255,255,255,0.8)" }}>�Ÿ“ˆ Scale-In Risk Budget</Typography>
            ...
            {capMode === "pct" && (
              <>
                ...
                {capDollarHint !== null && <Typography sx={{ fontSize:"11px", color:"rgba(56,189,248,0.6)", mt:"8px", fontFamily:'"Roboto Mono",monospace' }}>�‰ˆ ${capDollarHint.toFixed(2)} extra for scale-in</Typography>}
              </>
            )}
            {capMode === "dollar" && (
              <>
                ...
                {capPctHint !== null && <Typography sx={{ fontSize:"11px", color:"rgba(34,197,94,0.6)", mt:"8px", fontFamily:'"Roboto Mono",monospace' }}>�‰ˆ Total risk becomes {capPctHint.toFixed(4)}% after scale-in</Typography>}
              </>
            )}
          </Box>
        </Grid>
      ...
      {(() => {
        ...
        return (
          <Box sx={{ py:"14px", borderRadius:"14px", textAlign:"center", fontWeight:800, fontSize:"15px", background:ready?"linear-gradient(135deg,#0ea5e9,#6366f1)":"rgba(255,255,255,0.05)", color:ready?"white":"rgba(255,255,255,0.2)", border:ready?"none":"1px dashed rgba(255,255,255,0.1)", boxShadow:ready?"0 6px 24px rgba(56,189,248,0.25)":"none", cursor:ready?"pointer":"default", display:"flex", alignItems:"center", justifyContent:"center", gap:1.5, transition:"all 0.2s", mb:2 }}>
            {ready ? "�Ÿ“� Calculate Scale-In Lot �†’" : "Fill in all fields above to calculate"}
          </Box>
        );
      })()}
        ...
        <Box sx={{ display:"flex", flexDirection:"column", gap:2 }}>
          <Box sx={{ ...card(), p:"26px 28px", background:result.safe?"linear-gradient(135deg,rgba(34,197,94,0.1),rgba(13,22,37,0.95))":"linear-gradient(135deg,rgba(245,158,11,0.1),rgba(13,22,37,0.95))", border:result.safe?"1px solid rgba(34,197,94,0.3)":"1px solid rgba(245,158,11,0.3)" }}>
            <Box sx={{ display:"flex", alignItems:"center", gap:1.5, mb:"20px" }}>
              <Typography sx={{ fontSize:"18px" }}>{result.safe?"�œ…":"�š�️"}</Typography>
            ...
            return (
              <Box sx={{ ...card(), p:"18px 22px", border:passed?"1px solid rgba(34,197,94,0.2)":"1px solid rgba(239,68,68,0.2)" }}>
                <Box sx={{ display:"flex", alignItems:"center", gap:2, mb:"10px" }}>
                  <Typography sx={{ fontSize:"14px", fontWeight:800 }}>{passed?"�œ…":"�Œ"} {pf.name} �€” Combined Risk Check</Typography>
    ...
    <Box>
      ...
      {correlation_warnings.length > 0 && (
        <Box sx={{ mb:2, p:"14px 16px", borderRadius:"12px", background:"rgba(251,191,36,0.05)", border:"1px solid rgba(251,191,36,0.18)" }}>
          <Typography sx={{ fontSize:"11px", fontWeight:700, color:"#fbbf24", textTransform:"uppercase", letterSpacing:".08em", mb:1 }}>�š� Correlation Warnings</Typography>
...
function PortfolioTab({ liveBalance }: { liveBalance: number }) {
  ...
  return (
    <Box sx={{ maxWidth:"900px" }}>
      {/* Header */}
      <Box sx={{ mb:3 }}>
        <Typography sx={{ fontSize:"18px", fontWeight:800, color:"white", mb:"4px" }}>�Ÿ“Š Portfolio Exposure Tracker</Typography>
        ...
        {positions.map((pos, idx) => (
          <Box key={pos.id} sx={{ mb:1.5, p:"16px 18px", borderRadius:"14px", background:"rgba(255,255,255,0.025)", border:"1px solid rgba(255,255,255,0.07)" }}>
            <Box sx={{ display:"flex", alignItems:"center", mb:1 }}>
              ...
              {positions.length > 1 && (
                <Box onClick={() => removePos(pos.id)} sx={{ ml:"auto", p:"3px 8px", borderRadius:"6px", cursor:"pointer", fontSize:"11px", color:"rgba(239,68,68,0.5)", "&:hover":{ color:"#ef4444", background:"rgba(239,68,68,0.08)" } }}>�œ•</Box>
              )}
            </Box>
      ...
      {error && <Box sx={{ p:"10px 14px", mb:2, borderRadius:"10px", background:"rgba(239,68,68,0.08)", border:"1px solid rgba(239,68,68,0.25)" }}><Typography sx={{ fontSize:"12px", color:"#ef4444" }}>�š� {error}</Typography></Box>}
      ...
      <Box onClick={!loading?analyze:undefined} sx={{ py:"13px", borderRadius:"13px", textAlign:"center", cursor:loading?"not-allowed":"pointer", fontWeight:800, fontSize:"14px", mb:3, background:loading?"rgba(168,85,247,0.2)":"linear-gradient(135deg,#a855f7,#ec4899)", color:"white", display:"flex", alignItems:"center", justifyContent:"center", gap:1, opacity:loading?0.7:1, boxShadow:loading?"none":"0 6px 24px rgba(168,85,247,0.2)", transition:"all 0.15s" }}>
        {loading ? <><CircularProgress size={14} sx={{color:"white"}} /> Analysing�€�</> : "�Ÿ“Š Analyze Portfolio �†’"}
      </Box>
      ...
      <Box sx={{ mt:4, p:"22px 24px", borderRadius:"18px", background:"rgba(56,189,248,0.04)", border:"1px solid rgba(56,189,248,0.14)" }}>
        <Typography sx={{ fontSize:"15px", fontWeight:800, color:"white", mb:"3px" }}>�Ÿ�� Margin Calculator</Typography>
        ...
        <Box sx={{ display:"flex", gap:"10px", flexWrap:"wrap", mb:"16px", alignItems:"flex-end" }}>
          ...
          <Box onClick={calcMargin} sx={{ px:"18px", py:"11px", borderRadius:"11px", cursor:"pointer", fontWeight:800, fontSize:"13px", background:"rgba(56,189,248,0.12)", border:"1px solid rgba(56,189,248,0.3)", color:"#38bdf8", display:"flex", alignItems:"center", gap:"6px", transition:"all 0.15s", "&:hover":{ background:"rgba(56,189,248,0.2)" }, whiteSpace:"nowrap" }}>
            {marginLoading ? <CircularProgress size={13} sx={{color:"#38bdf8"}} /> : "�Ÿ��"} Calculate
          </Box>
        </Box>
...
export default function RiskCheck() {
  ...
  const tabs = [
    { k:"check",     icon:"�Ÿ”�", label:"Risk Check",          sub:"Evaluate a single trade"    },
    { k:"scalein",   icon:"�Ÿ“�", label:"Scale-In Calculator", sub:"Add to an open position"    },
    { k:"portfolio", icon:"�Ÿ“Š", label:"Portfolio Tracker",   sub:"Multi-position exposure"    },
  ] as const;
