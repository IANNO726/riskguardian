...
const AGENT_DOWNLOAD_URL =
  "https://github.com/IANNO726/riskguardian/releases/download/v1.0.0/RiskGuardianAgent.exe";
...
const AgentPage: React.FC = () => {
  ...
  const card = {
    ...
    borderRadius: "20px",
    color:        "white",           // ✅ force all child text white by default
  };
  ...
  return (
    <Box sx={{
      p: { xs: 2, md: 4 }, color: "white", minHeight: "100vh",
      ...
      <Box sx={{ mb: 4 }}>
        <Typography sx={{
          fontSize: { xs: 24, md: 32 }, fontWeight: 800, mb: 0.5,
      ...
      {/* ── Live Data Cards (shown only when connected) ─────────────────────── */}
      {connected && agentData && (
        <Box sx={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 2, mb: 3 }}>
          {[
            ...
            <Box key={i} sx={{
              ...card, p: 2.5, position: "relative", overflow: "hidden",
              "&::before": {
      ...
      <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "1fr 1fr" }, gap: 3 }}>
        ...
        <Card sx={{ ...card }}>
          <CardContent sx={{ p: 3 }}>
            ...
            <Typography sx={{ color: "rgba(255,255,255,0.55)", fontSize: 13, mb: 2.5, lineHeight: 1.6 }}>
              A small Windows app (~18MB) that runs alongside your MT5 terminal.
            ...
            <Box sx={{ mb: 2.5, p: 2, borderRadius: "12px", background: "rgba(56,189,248,0.06)", border: "1px solid rgba(56,189,248,0.15)" }}>
              {[
                ...
                "Reads MT5 data locally (no screen sharing)",
                "Encrypted HTTPS connection",
                "Works with any Deriv MT5 account",
              ].map((f, i) => (
            ...
            <Button
              ...
              sx={{
                ...
                color: "white",
                "&:hover": { transform: "translateY(-2px)", boxShadow: "0 8px 24px rgba(56,189,248,0.4)" },
              }}>
              Download RiskGuardian Agent (.exe)
            </Button>
            <Typography sx={{ fontSize: 11, color: "rgba(255,255,255,0.3)", textAlign: "center", mt: 1 }}>
              Windows 10/11 · ~18MB · No installation required
            </Typography>
          </CardContent>
        </Card>
        ...
        <Card sx={{ ...card }}>
          <CardContent sx={{ p: 3 }}>
            ...
            {!hasToken ? (
              <Button
                onClick={generateToken} disabled={loading} fullWidth
                sx={{
                  py: 1.5, borderRadius: "12px", textTransform: "none",
                  fontWeight: 700, fontSize: 14,
              ...
              <>
                <Box sx={{
                  mb: 2, p: 2, borderRadius: "12px", background: "#111827",
                  ...
                  {fullToken && (
                    <Tooltip title={copied ? "Copied!" : "Copy token"}>
                      <IconButton
                        onClick={copyToken} size="small"
                        sx={{ color: copied ? "#22c55e" : "rgba(255,255,255,0.4)" }}>
                ...
                {!fullToken && (
                  <Typography sx={{ fontSize: 12, color: "rgba(255,255,255,0.35)", mb: 2, textAlign: "center" }}>
                    Token hidden for security. Regenerate to get a new one.
                  </Typography>
                )}
                <Button
                  onClick={generateToken} disabled={loading} fullWidth variant="outlined"
                  startIcon={<RefreshIcon />}
                  sx={{
                    py: 1.2, borderRadius: "12px", textTransform: "none", fontWeight: 600,
          ...
          <CardContent sx={{ p: 3 }}>
            <Box sx={{ display: "flex", alignItems: "center", gap: 1.5, mb: 2.5 }}>
              <Box sx={{
                ...
                display: "flex", alignItems: "center", justifyContent: "center",
                fontSize: 16, fontWeight: 800, color: "#a855f7",
              }}>3</Box>
              <Typography sx={{ fontWeight: 700, fontSize: 16, color: "white" }}>
                Run the Agent
              </Typography>
            ...
            <Box sx={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 2 }}>
              {[
                { icon: "📥", title: "Download",  desc: "Download RiskGuardianAgent.exe and save it anywhere on your PC" },
                { icon: "🔑", title: "Get Token", desc: "Generate your token above and copy it" },
                { icon: "▶️", title: "Run Agent", desc: "Double-click the .exe, paste your token, click Start Agent" },
                { icon: "📊", title: "Live Data", desc: "Your dashboard instantly shows live equity, positions and P&L" },
              ].map((item, i) => (
                <Box key={i} sx={{
                  ...
                  <Typography sx={{ fontWeight: 700, fontSize: 14, color: "white", mb: 0.5 }}>
                    {item.title}
                  </Typography>
            ...
            <Box sx={{
              ...
              <Typography sx={{ fontSize: 13, color: "rgba(255,255,255,0.65)" }}>
                💡 <strong style={{ color: "#f59e0b" }}>Tip:</strong> Add the agent to Windows Startup
                so it runs automatically. Right-click the .exe → Create shortcut → Move shortcut to{" "}
