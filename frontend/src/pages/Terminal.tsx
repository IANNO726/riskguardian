  ...
  return (
    <Box sx={{
      ...
      <Grid container spacing={{ xs: 1.5, md: 2.5 }} sx={{ mb: { xs: 3, md: 4 } }}>
        ...
        {/* Daily P&L */}
        <Grid item xs={6} md={3}>
          <Box sx={{ p: { xs: 2, md: 3 }, borderRadius: '20px', background: liveProfit >= 0 ? 'linear-gradient(135deg,rgba(34,197,94,0.1),rgba(34,197,94,0.03))' : 'linear-gradient(135deg,rgba(239,68,68,0.1),rgba(239,68,68,0.03))', border: `1px solid ${liveProfit >= 0 ? 'rgba(34,197,94,0.22)' : 'rgba(239,68,68,0.22)'}`, position: 'relative', overflow: 'hidden', transition: 'all 0.3s', '&:hover': { transform: 'translateY(-4px)', boxShadow: `0 16px 40px ${liveProfit >= 0 ? 'rgba(34,197,94,0.15)' : 'rgba(239,68,68,0.15)'}` }, '&::before': { content: '""', position: 'absolute', top: 0, left: 0, right: 0, height: '2px', background: `linear-gradient(90deg,transparent,${liveProfit >= 0 ? '#22c55e' : '#ef4444'},transparent)` } }}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <Box>
        ...
        <Box sx={{ px: { xs: 2, md: 4 }, pt: 3, pb: 2.5, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 2, borderBottom: '1px solid rgba(255,255,255,0.06)', background: 'rgba(56,189,248,0.03)' }}>
          ...
          {positions.length > 0 && (
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
              ...
              <Box sx={{ px: 2, py: 0.8, borderRadius: '10px', background: totalPositionLoss < 0 ? 'rgba(239,68,68,0.12)' : 'rgba(255,255,255,0.05)', border: '1px solid rgba(239,68,68,0.25)', display: 'flex', alignItems: 'center', gap: 0.8 }}>
